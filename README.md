...
- **Time-aware exposure (v0.2)** — the differentiator. From the doses you *actually logged*:
  - **Estimated active windows** per ingredient ("Ibuprofen active until ~7:36 PM"), derived from typical adult elimination **half-lives** (window ≈ 5 half-lives — the standard washout convention, deliberately conservative so a drug counts as "present" until it's essentially gone). Where the clinical effect outlasts the drug level — aspirin's irreversible antiplatelet effect, warfarin's clotting-factor turnover, nitrate-separation labels — the window models the **effect duration** instead, and the UI says which basis was used. Extended-release products are detected and excluded from window modeling.
...
- **Drug shortage awareness (v0.15)** — Canada-specific, and the reason being Canadian-first pays off. Medications are matched by DIN against Health Canada's shortage and discontinuation database, so a pharmacist sees "3 patients are on a product that's short" *before* someone walks in to an empty shelf, and patients see a badge on the medication itself. It is deliberately informational: shortage data never creates an alert, never changes a severity, and the app never suggests a substitute — that's a pharmacist's call. It also refuses to bluff: every medication reports whether it was **checked** (with the date, and a warning once that date goes stale), **never checked**, **can't be checked** (unverified entries have no DIN), or **not set up on this instance** — because "no shortage shown" must never be mistaken for "supply is fine". Needs free credentials from drugshortagescanada.ca (`DSC_EMAIL` / `DSC_PASSWORD`); without them the feature says so plainly instead of showing false all-clears.
- **Pharmacy organizations (v0.14)** — the multi-staff pharmacy model. An owner creates their organization (permanently labeled **"Unverified organization"** until a real verification process exists), adds locations, and invites staff with one-time hashed join links carrying a fixed role (owner/admin manage; pharmacists review; staff view) — an invite minted by someone who later loses their standing dies automatically. Access requests can now be made **on behalf of the organization + location**: the patient's consent card names the org and says plainly that approval shares the record with *everyone there, including future staff*, and one revoke cuts the whole team off. The consent boundary is strict: **existing individual grants never silently widen into org access** (a team request is always a new, patient-approved card), a deactivated staff member loses every org patient on their very next request while their own personal consents survive, and membership is re-checked on every single read. Security notes: join links are bearer tokens (hand them over directly), unverified orgs mean patients must recognize the pharmacy before approving, and losing the sole owner account currently requires manual recovery — all part of the clinic-deployment hardening list.
- **Pilot-ready reporting & metrics (v0.13)** — the pharmacist report now tells the whole story: who has access and why (patient's own report only), each medication's provenance and reconciliation disposition ("Confirmed correct as listed — Dr. X, date"), actual-use callouts, adherence including missed doses with the formula printed, and the exact ruleset/exposure versions behind every claim. The clinic dashboard gains a **pilot metrics** panel: invitation funnel with completion rate, median invite→consent and consent→review times, reconciliation throughput and discrepancy density, and safety counts — all **PHI-free aggregates** scoped to the signed-in professional (test-enforced: no patient identifier can appear in the payload), with unmeasured metrics listed explicitly so a missing number is never mistaken for zero.
- **Reconciliation workspace (v0.12)** — the MedsCheck-prep tool the earlier phases were building toward. For each medication the system computes **deterministic discrepancy flags** — unverified entry, "patient reports not taking this", taken-differently, duplicate ingredient, open unreviewed interaction, missed doses, missing dose/schedule — and the pharmacist works through them in a session, recording a **disposition** per medication (confirmed correct / discrepancy resolved / needs follow-up / patient unsure / referred to prescriber), each stamped with who and when. The system **never decides which list is correct** — flags inform, humans judge. Confirming a medication marks it **pharmacy-confirmed** on the record; if the patient later changes its dose, that badge honestly reverts to patient-reported. Sessions complete with coverage ("8 of 10 reviewed") and a summary note; the patient sees the whole history in their audit trail, and the report shows the last reconciliation.
- **Real adherence (v0.11)** — the app now knows what *should* have been taken, not just what was logged. Expected doses are generated deterministically from each medication's schedule **as it was on each date** (schedule edits are historized, so changing your times today never rewrites yesterday's expectations), matched against logged doses, and classified taken-on-time / late / skipped / **missed** / not-yet-due. As-needed (PRN) medications are never "missed". The percentage formula is printed right on the report — (on-time + late) ÷ all due doses — and when no doses were due, the app says **"no data", never a misleading 0%**. Surfaced on the pharmacist report (per-med table), the clinic roster and patient record, and as a dashboard summary strip.
- **Medication editing with immutable identity (v0.10)** — patients can edit how they take a medication (dose, unit, schedule, PRN, start date, instructions) but never what it *is*: the Health-Canada-verified product identity is immutable, and switching products is an explicit **Replace** action that keeps the old entry as stopped history (dose logs intact, supersession link recorded) and re-runs every safety check against the new product. Clinician reviews deliberately survive dose edits — the combination hasn't changed — but gain an amber **"dose or schedule changed after this review"** warning with the change date, while a product replacement breaks the review's identity so the pair re-alerts. Every record also carries a server-owned provenance label ("Patient-reported" today; pharmacy-confirmed arrives with reconciliation).
