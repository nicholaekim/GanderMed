...
- Sync is explicit (`POST /api/shortages`, clinician-only, rate-limited): distinct DINs on active verified meds, skipping those checked in the last 12h, capped per run; whatever is skipped stays honestly `never_checked`. `GET /api/shortages` is a normal profile-scoped read via resolveProfileAccess.
