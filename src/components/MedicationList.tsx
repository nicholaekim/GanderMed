...
import { LIFECYCLE_LABELS, effectiveLifecycle, isCurrentlyTaken, isPaused, isPlanned, isTerminal } from "@/lib/lifecycle";
...
function MedCard({
  ...
  const planned = isPlanned(med);
  const ended = isTerminal(med);
  const stopped = ended || planned; // muted styling for anything not being taken
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500">
            ...
            {lifecycle !== "currently_taking" && lifecycle !== "stopped" && (
              <span
            ...
            {!ended && !planned && <SupplyBadge supply={supply} compact />}
            {med.actual_use && med.actual_use !== "taking" && isCurrentlyTaken(med) && (
              <span
        ...
        {!readOnly && !planned && (
          <div className="flex shrink-0 gap-1.5">
            ...
            {/* Only a plainly-stopped med can be toggled back on; declined /
                cancelled / replaced / completed are closed records (the
                server 409s a resurrect attempt). */}
            {lifecycle === "stopped" ? (
              <button
      ...
      {med.prescribed_at && (
        <div className="mt-1.5 rounded-md bg-slate-50 px-2 py-1.5 text-[11px] text-slate-600">
          ...
          {med.prescribed_is_prn
            ? "as needed"
            : (JSON.parse(med.prescribed_schedule_times ?? "[]") as string[]).join(", ") || "no set times"}
          {med.prescribed_instructions ? ` · ${med.prescribed_instructions}` : ""} ({fmtDate(med.prescribed_at)}
          {med.prescriber_name ? ` by ${med.prescriber_name}` : ""})
          {isCurrentlyTaken(med) &&
            (med.dose_value !== med.prescribed_dose_value ||
              med.schedule_times !== (med.prescribed_schedule_times ?? "[]")) && (
              <span className="block font-semibold text-amber-800">
                Patient reports: {med.dose_value != null ? `${med.dose_value} ${med.dose_unit ?? ""} · ` : ""}
                {scheduleLabel(med.is_prn, med.schedule_times)}
              </span>
            )}
        </div>
      )}
      {med.source_type === "clinician_medication_plan" && (
        <p className="mt-1 text-[11px] text-slate-500">
          Entered by {med.source_user_name ?? "a care-team member"} — based on a prescription created
          outside GanderMed.
        </p>
      )}
      {planned && !readOnly && (
        <p className="mt-1.5 text-[11px] text-violet-700">
          Waiting for your confirmation — use the &ldquo;New medication plans&rdquo; panel above.
        </p>
      )}
      <p className="mt-1 text-[11px] text-slate-400">
        {planned
          ? `Plan entered ${fmtDate(med.created_at)}`
          : ended && lifecycle !== "stopped"
            ? `${LIFECYCLE_LABELS[lifecycle]} ${fmtDate(med.end_date)}`
            : ended
...
export default function MedicationList({
  ...
  const current = meds.filter((m) => isCurrentlyTaken(m) || isPaused(m));
  const planned = meds.filter((m) => isPlanned(m));
  const ended = meds.filter((m) => isTerminal(m));

  const grid = (list: Medication[]) => (
  ...
  return (
    <section>
      <h2 className="mb-2 text-base font-semibold">Current medications</h2>
      {current.length === 0 ? (
      ...
      {planned.length > 0 && (
        <div className="mt-4">
          <h3 className="mb-2 text-sm font-semibold text-violet-800">Planned — not yet started</h3>
          {grid(planned)}
        </div>
      )}
      ...
      {ended.length > 0 && (
        <div className="mt-3">
          ...
          >
            {showEnded ? "Hide" : "Show"} {ended.length} stopped, completed, cancelled or declined
          </button>
          {showEnded && <div className="mt-2">{grid(ended)}</div>}
        </div>
      )}
    </section>
  );
